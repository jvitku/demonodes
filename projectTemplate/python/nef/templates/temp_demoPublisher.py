...
# node utils..
title='temp_demoPublisher'
label='temp_demoPublisher'
icon='temp_demoPublisher.png'

# parameters for initializing the node
params=[
('name','Select name for NeuralModule interfacing the temp_demoPublisher',str),
...
def make(net,name='NeuralModule which interfaces temp_demoPublisher with the Nengo simulator', independent=True, useQuick=True):
    ...
    g.addNode(pub, "temp_publisher", "java");  				# start java node and name it subscriber in the ROS network
