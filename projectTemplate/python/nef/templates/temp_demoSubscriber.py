...
# node utils..
title='temp_demoSubscriber'
label='temp_demoSubscriber'
icon='temp_demoSubscriber.png'

# Define the parameters for initializing the node
params=[
('name','Select name for NeuralModule interfacing the temp_demoSubscriber',str),
...
def make(net,name='NeuralModule which interfaces temp_demoSubscriber with the Nengo simulator', independent=True, useQuick=True):
    ...
    g.addNode(node, "temp_subscriber", "java");  	    # start java node and name it subscriber in the ROS network
    module = NeuralModule('temp_Subscriber_'+name, g) 	# Construct the neural module (small ROS network: node+modem )
